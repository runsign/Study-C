...
/*
 메모리 위계구조 (4가지는 물리적으로 구분이 된다 )
	1. 레지스터
	2. 캐시
	3. 메모리 (4가지는 논리적으로 구분이 된다)
			- 코드	 : 컴파일 타임에 결정되고 저장된다. 실행을 하면 기계어로 번역되어 저장된 코드를 읽어온다.  read-only
			- 데이터	 : 전역 /static 변수 등 프로그램이 사용하는 데이터가 저장되는 곳.
					   컴파일이 완료되면 코드는 전역 /static 변수의 주소값을 가리킨다 (포인팅)
					   런타임 도중에 읽고 쓸 수 있다.
			- 힙		 : 동적할당이 이루어지는 곳. 런타임 도중에 수시로 바뀐다.
			- 스택	 : 정적할당이 이루어지는 곳. 이 정정할당은 런타임에는 변경될 수 없다. (지역변수, 매개변수, 리턴값)
					   어느 실행지점에서 얼마만큼 할당할지는 이미 코드를 컴파일할 때에 정해져 있다. 그 상태에서 런타임으로 진입.
	4. 하드

	위로 갈수록 CPU에 가깝고 속도가 빠르고 비싸진다.
*/